...
u32 PCT_SendMsgToCloud(ZC_SecHead *pstruSecHead, u8 *pu8PlainData)
{
    ...
    if (MSG_BUFFER_IDLE == g_struSendBuffer.u8Status)
    {
        u16Len = ZC_HTONS(pstruSecHead->u16TotalMsg) + u16PaddingLen;

        /*first check padding,then Encrypt, final copy sechead*/
        u32RetVal = SEC_Encrypt(pstruSecHead, g_struSendBuffer.u8MsgBuffer + sizeof(ZC_SecHead), pu8PlainData, &u16Len);
        ...
        /*copy sechead*/
        memcpy(g_struSendBuffer.u8MsgBuffer, (u8*)pstruSecHead, sizeof(ZC_SecHead));
    
        g_struSendBuffer.u32Len = u16Len + sizeof(ZC_SecHead);
        g_struSendBuffer.u8Status = MSG_BUFFER_FULL;
#if 0
        /* Ö±½Ó·¢ËÍ */
        ...
        g_struProtocolController.pstruMoudleFun->pfunSendTcpData(g_struProtocolController.struCloudConnection.u32Socket,
                                                                    g_struSendBuffer.u8MsgBuffer, 
                                                                    g_struSendBuffer.u32Len,
                                                                    &struParam);
     
        ZC_Printf("@send data len = %d\n", g_struSendBuffer.u32Len);  
        g_struSendBuffer.u8Status = MSG_BUFFER_IDLE;
        g_struSendBuffer.u32Len = 0;    
#endif        
        MSG_PushMsg(&g_struSendQueue, (u8*)&g_struSendBuffer);
